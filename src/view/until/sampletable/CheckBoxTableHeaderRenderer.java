package view.until.sampletable;
...
public class CheckBoxTableHeaderRenderer extends JCheckBox implements TableCellRenderer {
    ...
    private void init() {
        if (table == null || column < 0 || column >= table.getColumnCount()) {
            return; // Không thực hiện gì nếu table không hợp lệ
        }
    ...
    private void checkRow() {
        if (table == null || column < 0 || column >= table.getColumnCount() || table.getRowCount() == 0) {
            return; // Không thực hiện gì nếu table không hợp lệ hoặc không có hàng
        }

        Object initValueObj = table.getValueAt(0, column);
        if (!(initValueObj instanceof Boolean)) {
            return; // Không thực hiện gì nếu giá trị không phải boolean
        }

        boolean initValue = (Boolean) initValueObj;

        for (int i = 1; i < table.getRowCount(); i++) {
            Object valueObj = table.getValueAt(i, column);
            if (!(valueObj instanceof Boolean)) {
                continue; // Bỏ qua nếu giá trị không phải boolean
            }

            boolean v = (Boolean) valueObj;
            if (initValue != v) {
