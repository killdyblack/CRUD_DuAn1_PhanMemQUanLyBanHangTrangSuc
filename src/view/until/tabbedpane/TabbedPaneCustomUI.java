...
import view.until.tabbedpane.TabbedPaneCustom;
...
public class TabbedPaneCustomUI extends FlatTabbedPaneUI {
    ...
    protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex, int x, int y, int width, int height, boolean isSelected) {
        // No custom border painting
    }

    @Override
    protected void paintFocusIndicator(Graphics g, int tabPlacement, Rectangle[] rects, int tabIndex, Rectangle iconRect, Rectangle textRect, boolean isSelected) {
        // No custom focus indicator painting
    }
    ...
    private Color getTabColor(boolean selected) {
        if (selected) {
            Color color1 = Color.decode("#FEE715");
            return color1; // Set selected tab color to yellow
        } else {
